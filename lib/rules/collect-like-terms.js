...
/*
  Given any node, return a dictionary with variable keys and coefficient values.
  Also return a list of all the constant terms.
  Only when all the variables are polynomialTerms, not polynomials.

  e.g. 12x^2 + 5x + 2 -> {coefficientMap: [x^2 : 12, x : 5], constants: [2]}
*/

export const getCoefficientsAndConstants = (node, coefficientMap = {}, constants = []) => {
    if (query.isNumber(node)) {
        constants.push(node)
        ...
        node.args.forEach(arg => {
            getCoefficientsAndConstants(arg, coefficientMap, constants)
        })
    } 
