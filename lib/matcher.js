...
// TODO: simplify this using 'replace' from math-traverse
    ...
    return patternKeys.every(key => {
        if (key === 'args' && isVariableLengthPattern(pattern)) {
            ...
            if (query.isMul(pattern) && query.isMul(input) || query.isAdd(pattern) && query.isAdd(input)) {
                for (let i = 0; i <= input.args.length - 2; i++) {
                    ...
                    // This is kind of roundabout but we're popuplate the
                    // first pattern using the placeholders for that pattern
                    // but updating the variable placeholder's subscript each
                    // time.
                    // TODO: simplify this using 'replace' from math-traverse
                    firstPlaceholders[variablePlaceholder.name] = {
                        '0': variablePlaceholder,
                    ...
                    for (j = i; j < input.args.length; j++) {
                        variablePlaceholder.subscript.value = String(subscript++)
...
export const applyRule = (rule, input) => {
    ...
    if (matchedNode) {
        const replacement = fixMinuses(rewriteFn(matchedNode, placeholders))
