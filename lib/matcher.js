...
import {print, evaluate} from 'math-parser'
            ...
            if (query.isMul(pattern) && query.isMul(input) || query.isAdd(pattern) && query.isAdd(input)) {
                for (let i = 0; i < input.args.length - 1; i++) {
                    ...
                    for (j = i; j < input.args.length; j++) {
                            ...
                            clone(firstPattern),
                            {
                                leave(node) {
                                    if (node.type === 'Placeholder' && node.subscript) {
                                        ...
                                        // Get a reference to the variable place
                                        // e.g. #a_1, that's currently being
                                        // matched.  We may need to update its
                                        // value in placeholdersCopy.
                        ...
                        const arg = query.isAdd(pattern) && query.isNeg(input.args[j])
                            ? input.args[j].args[0]
                            : input.args[j]
                        ...
                        // If we unwrapped an arg that was a 'neg' node, then
                        // we'll need to re-apply the 'neg' to the node that
                        // was matched in placeholdersCopy.
                        if (query.isAdd(pattern) && query.isNeg(input.args[j]) && variablePlaceholder) {
                            const name = variablePlaceholder.name
                            const subscript = variablePlaceholder.subscript.value
                            const node = placeholdersCopy[name][subscript]
                            placeholdersCopy[name][subscript] =
                                build.applyNode(
                                    'neg',
                                    [node],
                                    {wasMinus: true})
