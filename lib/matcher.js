...
const isVariableLengthPattern = (pattern) => {
    // TODO: also check if:
...
const expandVariableLengthPattern = (pattern, placeholders) =>  {
    const outputPattern = clone(pattern)
...
export const rewrite = (rule, input) => {
    ...
    if (matchedNode) {
        const replacement = isFunction(rewritePattern)
            ? rewritePattern(matchedNode, placeholders)
            : populatePattern(
                expandVariableLengthPattern(rewritePattern, placeholders),
                placeholders)
