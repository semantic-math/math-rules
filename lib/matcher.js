 ...
 * @param {node} subtree of input to be matched, TODO: handle when node is an array
 ...
 */
const getNodeAtPath = (ast, path) => {
    if (!ast) {
        throw new Error('node doesn\'t exist at path')
    } else if (path.length > 0) {
        const [key, ...restOfPath] = path
        return getNodeAtPath(ast[key], restOfPath)
    } else {
        return ast
    }
}
...
export const applyRule = (rule, input) => {
    ...
    const output = {}
    output['inputPath'] = getPath(input, matchedNode)
    ...
    if (matchedNode) {
        ...
        output['outputPath'] = getPath(result, replacement)
        output['outputAST'] = result
        console.log(output)
        return result
    } else {
        return input
