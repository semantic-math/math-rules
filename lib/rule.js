...
import {match, matchNode} from './matcher'
...
export const rewriteNode = (rule, node) => {
    const {matchFn, rewriteFn, constraints} = rule
    const result = matchFn(node)
    if (result) {
        const {placeholders, indexes} = result
        const replacement = fixMinuses(rewriteFn(node, placeholders, indexes))

        if (indexes && checkBounds(indexes, node.args)) {
            const {start, end} = indexes
            const newNode = clone(node)
            newNode.args.splice(start, end - start, clone(replacement))
            return newNode
        } else {
            return clone(replacement)
        }
