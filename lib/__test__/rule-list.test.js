...
describe('rules', () => {
    ...
    suite('simplify terms', rules.SIMPLIFY_TERMS, [
        // denom.length == 0, num.length == 0 (when all factors cancel)
        ['((x+1)(x+2)) / ((x+1)(x+2))', '1'],
        ['(x y z) / (x y z)', '1'],
        // denom.length == 0, num.length == 1
        ['(12(x+1)) / (x+1)', '12'],
        ['(x y z) / (y z)', 'x'],
    ...
    suite('common denominators', rules.COMMON_DENOMINATOR, [ 
        ...
        */
        //TODO: fix these cases case
        //['2 / x + 3 / (x + 1)^2 + 4 / (x y^2) + 3 / 4 + 3 / 6', ''],
        //['2 / (4 * x) + 2 / y'],
        //['2 / (x * x) + 3 / (x * y) + 4 / (y * y)', ''],
    ])
