...
describe('matcher', () => {
    ...
    describe.only('match', () => {
        ...
        it('should return the path', () => {
            const result1 = matchString('#a + #b', '1 * (2 + 3)')
            assert.deepEqual(result1.path, ['args', 1])

            const result2 = matchString('#a + #b', '2 (x + 1) = y^2')
            assert.deepEqual(result2.path, ['args', 0, 'args', 1])
        })
    })
