...
export const SIMPLIFY_TERMS = defineRule(
    (node) => {
        return query.isDiv(node) && query.isMul(node.args[0]) ? {node} : null
    },
    ...
    (node) => {
        ...
        */
        ...
        let result = build.div(build.apply('mul', num), denom[0])
        ...
        if (denom.length == 0) {
            if (num.length == 0) {
                ...
                result = build.apply('mul', num, {implicit:true})
            }
        } else if (num.length == 0){
            result = build.div(build.number(1), ...denom)
        } else {
            result = build.div(build.mul(...num), denom[0])
        }
        console.log(result)
        */
