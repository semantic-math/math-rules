            ...
            for (var j in denom) {
                ...
                if (num[i] && denom[j]) {
                    ...
                    if (canApplyRule(SIMPLIFY_FRACTION, frac)) {
                        ...
                        if (query.getValue(newNum) == 1) {
                            delete num[i]
                        }
                        if (query.getValue(newDenom) == 1) {
                            delete denom[j]
                        }
                        num[i] = newNum
                        denom[i] = newDenom
    ...
    (node) => {
        ...
        */
        ...
        if(denoms.every(denom => query.isNumber(denom))) {
            ...
            console.log(print(newDenoms))
            console.log(nums)
            // TODO: refactor
            newNumerators = nums.map(
                (num, i) => flattenOperands(build.implicitMul(num, applyRule(CANCEL_LIKE_TERMS, build.div(newDenoms, denoms[i])))))
            console.log(newNumerators)
        }
