...
// e.g. (12(x+1)) / (x+1) -> 12 / 1
export const SIMPLIFY_TERMS = defineRule(
    (node) => {
        // Must be div node and either numerator or denominator is a mul node (or both)
        // Does not match in cases like ((x+1) / (x+1))

        return query.isDiv(node)
            && (query.isMul(node.args[0]) || query.isMul(node.args[1])) ? {node} : null
    },
    ...
    (node) => {
        ...
        */
        ...
        let num = query.isMul(numerator)
            ? numerator.args
            : [numerator]
        
        let denom = query.isMul(denominator)
            ? denominator.args
            : [denominator]
        ...
        if (denom.length == 0) {
            if (num.length == 0) {
                ...
                result = build.apply('mul', num, {implicit:isImplicit(numerator)})
            }
        } else if (num.length == 0){
            if (denom.length == 1) {
                result = build.div(build.number(1), denom[0])
            } else {
                result = build.div(build.number(1), build.apply('mul', denom, {implicit: isImplicit(denominator)}))
            }
        } else if (denom.length == 1) {
            if (num.length == 1) {
                result = build.div(num[0], denom[0])
            } else {
                result = build.div(build.apply('mul', num, {implicit:isImplicit(numerator)}), denom[0])
            }
        } else if (num.length == 1) {
            result = build.div(num[0], build.apply('mul', denom, {implicit: isImplicit(denominator)}))
        } else {
            result = build.div(build.apply('mul', num, {implicit: isImplicit(numerator)}), build.apply('mul', denom, {implicit: isImplicit(denominator)}))
        }
        return result
    }
)
...
export const lcm_helper = (arr, vars = {}) => {
    ...
    arr.forEach(function(term){
        //console.log(isPolynomial(denom) || isPolynomialTerm(denom))
        if(query.isNumber(term)){
            LCM = lcm(query.getValue(term), LCM)
...
export const COMMON_DENOMINATOR = defineRule(
    ...
    (node) => {
        ...
        /*
          ...
          Else the new denom is the LCM of all the terms
          We assume here that all arithmetic has been simplified.
          e.g. 2/(2 * 2 * x) + 2/3 -> 2/(4 * x) + 2/3
        */
        ...
        if(denoms.every(denom => query.isNumber(denom))) {
            ...
            const {vars, LCM} = lcm_helper(denoms)

            // newDenoms is the product of all terms in var
            if (LCM == 1) {
