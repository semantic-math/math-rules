...
export const COMMON_DENOMINATOR = defineRule(
    (node) => {
        if (query.isAdd(node)) {
            ...
            if (terms.every(term => query.isNeg(term)
                            ? query.isDiv(term.args[0])
                            : query.isDiv(term))) {
                const denom = terms[0].args[1]
                sameDenom = terms.every(term => query.isNeg(term)
                                        ? print(term.args[0].args[1]) == print(denom)
                                        : print(term.args[1]) == print(denom))
            }
    ...
    (node) => {
        ...
        // add a denominator of 1 when there is a number or identifier
        terms = terms.map(term => isPolynomialTerm(term)
