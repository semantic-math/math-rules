...
export const EVALUATE_DISTRIBUTED_NTH_ROOT = defineRule(
    (node) => {
        ...
        let canEvaluate = canApplyRule(CANCEL_EXPONENT, node)
                ...
                flatten(Object.keys(count).map(key => {
                    ...
                    args.map(arg => {
                        if (Array.isArray(arg)) {
                            const loc = args.indexOf(arg)
                            args[loc] = build.applyNode('mul', arg)
                        }
                    })
...
// e.g 2 -> -2
/*
export const NEGATIVE = defineRule(
    (node) => {
        return isPolynomialTerm(node) || query.isNumber(node) ? {node} : null
    },
    (node) => {
        return build.applyNode('neg', [node], {wasMinus: true})
    }
)
*/

export const NEGATIVE = defineRule('#a', '-#a', {a: query.isNumber})
...
export const FACTOR_DIFFERENCE_OF_SQUARES = defineRule(
    (node) => {
