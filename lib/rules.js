...
export const ADD_EXPONENT_OF_ONE = defineRule(
    (node) => {
        ...
        return isMulOfPolynomials ? {node} : null
    },

    (node) => {
        const result = replace(node, {
            leave(node){
                if(query.isMul(node)){
                    let i = 0
...
// ROOTS
export const CANCEL_EXPONENT =
    defineRuleString('nthRoot(#a, #b)', )

export const NTH_ROOT_VALUE =
    defineRuleString('nthRoot(#a)', '#eval(#a ^ (1/2))')
