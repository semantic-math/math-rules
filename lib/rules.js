...
import {gcd, lcm} from 'math-evaluator'
...
*/
export const SIMPLIFY_FRACTION = defineRule(
    (node) => {
        return query.isDiv(node) ? {node} : null
    },
    ...
    (node) => {
        ...
        // Case #4: 0/3 -> 0
        // Case #5: 3/6 -> 1/2
        if (query.isNeg(numerator) && query.isNeg(denominator)) {
            let fraction = applyRule(CANCEL_MINUSES, node)
            return applyRule(SIMPLIFY_FRACTION, fraction)
        } else if (query.isNeg(numerator)) {
            // e.g -1/3 -> 1/3
            let fraction = parse(`${print(numerator.args[0])} / ${print(denominator)}`)
            // simplify the positive fraction
            let simplified = applyRule(SIMPLIFY_FRACTION, fraction)
            // add the negative sign
            return parse(`-${print(simplified)}`)
        } else if (query.isNeg(denominator)) {
            // e.g 1/-3 -> -1/3
            let fraction = parse(`-${print(numerator)} / ${print(denominator.args[0])}`)
            return applyRule(SIMPLIFY_FRACTION, fraction)
        } else if (query.getValue(numerator) == 0){
            return build.numberNode(0)
        } else {
            let numVal = query.getValue(numerator)
            let denomVal = query.getValue(denominator)
            let newNumerator = build.numberNode(numVal / gcd(numVal, denomVal))
            let newDenominator = build.numberNode(denomVal / gcd(numVal, denomVal))
            return build.applyNode('div', [newNumerator, newDenominator])
        }
    }
)
...
export const CANCEL_EXPONENT = defineRule(
    ...
    (node) => {
        ...
        // checks whether index is provided
        // if not assume square root
        // e.g. nthRoot(x^2)
        if (node.args[1]) {
            index = node.args[1]
        } else {
            index = build.numberNode(2)
        }
        ...
        if ((numVal > denomVal || numVal == -1) && denomVal == 1) {
            return build.applyNode('pow', [variable, numerator])
    ...
    (node) => {
        const result = build.applyNode(
            ...
            node.args.map(nthRoot => {
                if (query.isNumber(nthRoot.args[0])) {
...
export const GROUP_TERMS_BY_ROOT = defineRule(
    (node) => {
        let canGroupTerms = false
        canGroupTerms = canApplyRule(DISTRIBUTE_NTH_ROOT, node)
    ...
    (node) => {
        ...
        radicand.args.forEach(arg => {
            const value = query.getValue(arg)
            ...
            [build.applyNode(
                ...
                flatten(Object.keys(count).map(key => {
                    ...
                    while (leftover - times > 0) {
                        leftover -= times
                        args.push(Array(times).fill(build.numberNode(key)))
                    }
                    leftover == 1
                        ? args.push(build.numberNode(key))
                        : args.push(Array(leftover).fill(build.numberNode(key)))
...
// e.g nthRoot(4) -> 2
export const NTH_ROOT_VALUE = defineRuleString('nthRoot(#a, #b)', '#eval(#a^(1/#b))', {a: query.isNumber, b: query.isNumber})
