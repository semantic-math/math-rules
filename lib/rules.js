...
export const FACTOR_PERFECT_SQUARE_TRINOMIALS = defineRule(
    ...
    (node) => {
        ...
        const secondFactor = thirdPoly
              ? `${Math.sqrt(thirdCoef)} ${getVariableFactorName(thirdPoly)}^${thirdDegree/2}`
              : Math.sqrt(query.getValue(thirdTerm))

        const result = query.isNeg(secondTerm)
              ? parse(`(${print(firstFactor)} - ${secondFactor})^2`)
              : parse(`(${print(firstFactor)} + ${secondFactor})^2`)
        return result
    }
)
...
export const FACTOR_SUM_PRODUCT_RULE = defineRule(
    (node) => {
        if (query.isAdd(node) && node.args.length === 3) {
            ...
            if (isTrinomial) {
                ...
                const secondCoef = query.isNeg(secondTerm)
                      ? secondTerm.args[0].args[0]
                      : secondTerm.args[0]

                const thirdCoef = isPolynomialTerm(thirdTerm) && !query.isNumber(thirdTerm)
                      ? query.isNeg(thirdTerm) ? query.getValue(thirdTerm.args[0].args[0]) : query.getValue(thirdTerm.args[0])
                      : query.getValue(thirdTerm)

                const thirdPoly = isPolynomialTerm(thirdTerm) && !query.isNumber(thirdTerm)
                      ? query.isNeg(thirdTerm) ? thirdTerm.args[0].args[1] : thirdTerm.args[1]
                      : null
                ...
                isFactorable = Number.isInteger(nthRoot(firstCoef, 2))
                    && Number.isInteger(nthRoot(Math.abs(thirdCoef), 2))
                    && firstDegree % 2 == 0
                    && thirdDegree ? thirdDegree % 2 == 0 : true
                    && (print(secondTerm) == print(flattenOperands(matchSecondTerm))
                        || print(secondTerm) == '-' + print(flattenOperands(matchSecondTerm)))
    ...
    (node) => {
        ...
        const secondCoef = query.isNeg(secondTerm)
              ? parseInt(`-${query.getValue(secondTerm.args[0].args[0])}`)
              : query.getValue(secondTerm.args[0])
        ...
        ('mul',
         combo.map(factor => {
             console.log(Math.sign(factor[1]) < 0)
             const firstFactor = parse(`${factor[0]} ${getVariableFactorName(firstPoly)}^${firstDegree/2}`)

             //console.log(parse('2x - 3'))
             const secondFactor = thirdPoly
                   ? Math.sign(factor[1]) > 0
                   ? parse(`${factor[1]} ${getVariableFactorName(thirdPoly)}^${thirdDegree/2}`)
                   : build.applyNode('neg', [parse(`${factor[1]} ${getVariableFactorName(thirdPoly)}^${thirdDegree/2}`)], {wasMinus: true})
                   : Math.sign(factor[1]) > 0
                   ? build.numberNode(factor[1])
                   : build.applyNode('neg', [build.numberNode(factor[1] / -1)], {wasMinus: true})
