...
export const CANCEL_EXPONENT = defineRule(
    (node) => {
        // Checks if op is nthRoot and radicand is a pow applyNode
        // if conditions pass, check if root can be simplified
        let canCancel = false
        if (query.isApply(node)) {
            let isNthRoot = (node.op == 'nthRoot')
            let validRadicand = query.isPow(node.args[0])
            if (isNthRoot && validRadicand) {
